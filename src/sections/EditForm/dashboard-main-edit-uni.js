...
class EditUni extends Component {
  ...
  async componentDidMount() {
    ...
    try {
      ...
      //var twoColumns = todoList.map((node) => [node.id, node.name]);
      var twoColumns = [];
      todoList.forEach(function(d) {
        twoColumns.push({
          name:"uni",
          value: d.id,
          label: d.name
...
handleDynamicChangeSelect = (e) => {
  console.log(e)
  let { name,value } = e;
  const activeItem = this.state.uniList.filter((val, i, a) => {return val.id==value;});
