...
import SidebarDashboardStudent from "../../components/SidebarDashboard/SidebarDashboardStudent";
import {
  Form,
  FormGroup,
  Input,
...
const EditStudent =(props) => {
  ...
  const [IELTSBanddata,setIELTSBand] = useState("")
...
let filtered = await todoList.filter(function(val, i, a) {return val.id==props.studentId;})
...
const res_ieltscert = await fetch('https://ci-gsc.com/ieltscert/?format=json');

const todoList_ieltscert = await res_ieltscert.json();
let filtered_ieltscert = await todoList_ieltscert.filter(function(val, i, a) {return val.email==filtered[0].email;})

setImage(filtered_ieltscert)
} catch (e) {
console.log(e);
}
}
...
useEffect(() =>  {
 ...
 const success = () => {
  ...
  const onBirthDateChange = selectedOption => {
    setBirthDate(selectedOption);
  };
  const onBirthMonthChange = selectedOption => {
    setBirthMonth(selectedOption);
  };
  const onBirthYearChange = selectedOption => {
    setBirthYear(selectedOption);
  };
  const onprevQualificationChange = selectedOption => {
    setprev_qualification_data(selectedOption);
  };
  const onIELTSBandChange = selectedOption => {
    setIELTSBand(selectedOption);
  };
  ...
  const onDesiredSubjectChange = selectedOption => {
    setDesiredSubject(selectedOption);
  };
...
const handleSubmit = (item) => { 
  ...
  item.gender=genderdata,
  item.birth_date=birth_date_data,
  item.birth_month=birth_month_data,
  item.birth_year=birth_year_data,
  item.address1=address1,
  item.address2=address2,
  item.prev_qualification=prev_qualification_data,
  item.IELTSBand=IELTSBanddata,
  item.Desiredlevel=Desiredleveldata,
  item.StudyDestination=StudyDestinationdata,
  item.IntendedSemester=IntendedSemesterdata,
  item.DesiredSubject=DesiredSubjectdata
                          ...
                          <div className="col-lg-6 mb-xl-0 mb-7">
                            <div className="form-group position-relative">
                              ...
                              <Select
                                ...
                                className="form-control pl-0 arrow-3 w-100 font-size-4 d-flex align-items-center w-100 "
                                border={false}
                                onChange={onprevQualificationChange}
                                value={prev_qualification_data}
                              />
                            </div>
                          </div>
                          <div className="col-lg-6">
                          ...
                          <div className="col-lg-6">
                            <div className="form-group position-relative">
                              ...
                              <Select
                                ...
                                className="form-control pl-0 arrow-3 w-100 font-size-4 d-flex align-items-center w-100 "
                                border={false}
                                onChange={onDesiredSubjectChange}
                                value={DesiredSubjectdata}
                              />
                              <span className="h-100 w-px-50 pos-abs-tl d-flex align-items-center justify-content-center font-size-6"></span>
                            </div>
                          </div>
