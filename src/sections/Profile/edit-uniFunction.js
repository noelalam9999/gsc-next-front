...
class EditUni extends Component {
...
filter = () => {
  ...
  const uniId = _this.props.uniId
  let filtered = this.state.todoList.filter(function(val, i, a) {return val.id==uniId;})
}
  
  componentDidMount() {

    fetch('https://ci-gsc.com/uni/?format=json')
      .then(response => response.json())
      .then(data=>this.setState({ todoList: data }),()=> filter());
      // const todoList = await res.json();
//     let filtered = todoList.filter(function(val, i, a) {return val.id==uniId;})
// const {  email,
// name,
// mobile,
// country,
